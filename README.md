...
A full-stack AI-powered React app generator where you describe what you want to build and the AI writes production-ready React code that renders live in the browser — just like Bolt.new or Lovable.
...
Users describe an app they want to build, and the AI generates production-ready React code with a live Sandpack preview. The project includes persistent chat history, image upload support, and a credit-based subscription system. Pro users can trigger an AI agent that autonomously improves the generated app file by file.
...
- Features section, how-it-works steps, and pricing table (Clerk `<PricingTable />`)
...
- Cline Agent with two tools: `update_file` and `done_improving`
...
### Fix with AI
- Sandpack listens for runtime and compile errors
- Error banner appears in Preview tab with "Fix with AI" button
- Injects the error and context into Gemini and triggers a new generation
...
```bash
git clone https://github.com/Abhinavaggarwal7/ai-based-app-builder.git
cd ai-based-app-builder
npm install
```
...
Create a `.env.local` file in the root (use `.env.example` as a reference):
...
NEXT_PUBLIC_CLERK_STARTER_PLAN_ID=
